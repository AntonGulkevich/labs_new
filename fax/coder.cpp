...
QString Coder::code(QString value) {
    ...
    for (QStringList::Iterator series = lines.begin(); series != lines.end(); ++series) {
        ...
        int divisor = round(log2(L));
        tmp1.append(QString("%1").arg(QString::number(0, 2), divisor, QChar('0')));
        ncode += divisor;
    ...
    LOG << "n: " << n;
    LOG << "ncode: " << ncode;
    koef_->pos = n * 1.0 / ncode;
    /***********************************************************************************/
    /* Haffman */
    n = 0;
    ncode = 0;
    QString tmp2 = QString::null;
    tmp2.append("000000000001");
    ncode += 12;
    LOG << "ncode + 12";
    for (QStringList::Iterator series = lines.begin(); series != lines.end(); ++series) {
        QStringList s = series->split(" ");
...
QString Coder::getRLE() const {
    return QString::number(koef_->rle);
}

QString Coder::getPos() const {
    return QString::number(koef_->pos);
}
